...
# Determine the full path to the Bash interpreter
...
if ! crontab -l | grep -q "run_scheduled_task.sh"; then
  ...
  (crontab -l ; echo "0 10 * * 1-5 $bash_path $script_dir/run_scheduled_task.sh") | crontab -
  (crontab -l ; echo "30 10 * * 1-5 $bash_path $script_dir/run_scheduled_task.sh") | crontab -
fi
